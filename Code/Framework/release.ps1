...
$package_files = Get-ChildItem Framework\include\*.h,Framework\lib\*.lib,Framework\*.props,FrameworkSample\*.c,FrameworkSample\*.vcxproj,FrameworkSample\*.filters,FrameworkSample.sln -Recurse

$package_path = Join-Path $PSScriptRoot "Framework.zip"
...
foreach ($file in $package_files) {
    $relative_path = $(Resolve-Path -Path $file -Relative)	-replace '\.\\','Framework\'
