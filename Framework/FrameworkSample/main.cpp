...
static const ga_graphics_color ButtonColor			= ga_graphics_color_rgb( 0x05u, 0x04u, 0xaau );
static const ga_graphics_color ButtonBorderColor	= ga_graphics_color_rgb( 0xa2u, 0xcfu, 0xfeu );
static const ga_graphics_color ButtonIconColor		= ga_graphics_color_rgb( 0xa2u, 0xcfu, 0xfeu );
static const ga_graphics_color MouseColor			= ga_graphics_color_rgb( 0x13u, 0xeau, 0xc9u );
...
bool doButton( ga_graphics_t* pGraphics, ga_input_t* pInput, float centerX, float centerY, float width, float height, float& state, double gameTime )
{
...
int __stdcall WinMain( _In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPSTR lpCmdLine, _In_ int nShowCmd )
{
	...
	while( running && ga_graphics_is_window_open( pGraphics ) )
	{
		...
		// play button
		const float playButtonCenterY = halfWindowHeight - (ButtonHeight * 0.75f);
		if( doButton( pGraphics, pInput, halfWindowWidth, playButtonCenterY, ButtonWidth, ButtonHeight, playButtonState, gameTime ) )
		{
			ga_sound_play( pSound, pSoundFile );
		}

		ga_graphics_draw_triangle(
			pGraphics,
			halfWindowWidth - (ButtonIconSize * 0.5f),
			playButtonCenterY - (ButtonIconSize * 0.5f),
			halfWindowWidth + (ButtonIconSize * 0.5f),
			playButtonCenterY,
			halfWindowWidth - (ButtonIconSize * 0.5f),
			playButtonCenterY + (ButtonIconSize * 0.5f),
		...
		// mouse cursor
		const float mouseTime1 = float( -gameTime + M_PI * 2.0 * 0.333 );
		const float mouseTime2 = float( -gameTime );
		const float mouseTime3 = float( -gameTime + M_PI * 2.0 * 0.666 );
		const float mouseX		= ga_input_get_mouse_position_x( pInput );
		const float mouseY		= ga_input_get_mouse_position_y( pInput );
